...
export default function Duck() {
  ...
  async function handleStartSession(e: React.SubmitEvent) {
    ...
    try {
      ...
      addMessage(
        "duck",
        "Quack! Something went wrong starting the session. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function handleSend(e?: React.SubmitEvent) {
    ...
    try {
      ...
      const data = await res.json();
      addMessage("duck", data.duckQuestion);
      if (data.currentTeachingScore != null)
        setTeachingScore(data.currentTeachingScore);
    } catch (err) {
  ...
  async function handleEvaluate() {
    ...
    try {
      ...
      addMessage(
        "duck",
        "Quack! I couldn't evaluate right now. Keep teaching!"
      );
    } finally {
        ...
        <aside className="flex flex-col items-center gap-6 w-56 flex-shrink-0 pt-4">
          ...
          {teachingScore !== null && status !== "idle" && (
            <div className="w-full rounded-2xl border p-4 text-center transition-colors duration-300 bg-white dark:bg-white/5 border-stone-200 dark:border-white/10">
              <p className="text-xs uppercase tracking-widest mb-1 text-stone-400 dark:text-neutral-500">
                Teaching Score
              </p>
          ...
          {status === "idle" && (
            <div className="flex-1 flex flex-col items-center justify-center gap-6">
              <div className="text-center">
                <h1 className="text-3xl font-bold mb-2 text-stone-900 dark:text-white">
                  What are we learning today?
                </h1>
                <p className="text-stone-500 dark:text-neutral-400 text-sm">
                  Enter a topic and your duck will start asking questions.
                </p>
              </div>
              <form
                  ...
                  >
                    <div className="flex-shrink-0">
                      {msg.role === "duck" ? (
                        ...
                        <div className="w-9 h-9 rounded-full bg-amber-400 flex items-center justify-center text-neutral-950 font-bold text-xs">
                          You
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <span className="text-xs text-stone-400 dark:text-neutral-600 px-1">
                        {msg.time}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-stone-100 dark:bg-white/10 flex gap-1 items-center">
                      {[0, 1, 2].map((i) => (
                        <span
                          key={i}
                          className="w-2 h-2 rounded-full bg-stone-400 dark:bg-neutral-500 animate-bounce"
